...
public class UserCacheService {
	...
	public void save(User user) {
		redisTemplate.opsForValue().set(makeKey(USERS_CACHE_PREFIX, String.valueOf(user.getId())), user,
				Duration.ofMinutes(TIME_TO_LIVE_FOR_USER_IN_MINUTES));
		log.info("Putting cache for {} with id {}...", USERS_CACHE_PREFIX, user.getId());

		redisTemplate.opsForValue().set(makeKey(USERS_CACHE_PREFIX, user.getEmail()), user,
				Duration.ofMinutes(TIME_TO_LIVE_FOR_USER_IN_MINUTES));
	...
	public List<User> getByIds(Collection<Long> ids) {
		...
		List<Object> usersWithNulls = Objects.requireNonNull(redisTemplate.opsForValue().multiGet(keys));
